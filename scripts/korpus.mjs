...
function fortschritt(name) {
  return (n, gesamt, einheit = 'Seiten') => stdout.write(`\r  ${name}: ${n}/${gesamt} ${einheit} `);
}
...
async function hole(name) {
  ...
  console.log(`  ${zahl.format(gut.length)} Rezepte${buch.gelesen ? ` aus ${zahl.format(buch.gelesen)} ${buch.gelesenAls || 'Seiten'}` : ''}, `
